...
# ── 4. Tạo lịch sử mới hoàn toàn (xoá sạch file cũ trên GitHub) ────────────
echo -e "\n${CYAN}▸ Bước 3: Tạo lịch sử mới & push...${NC}"

CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "main")

# Tạo orphan branch để xoá toàn bộ lịch sử cũ
git checkout --orphan _upload_tmp 2>/dev/null
git add -A
git commit -m "MIZAI_BOT $(date '+%Y-%m-%d %H:%M')" 2>/dev/null

echo -e "  ${CYAN}Đang push lên GitHub (xoá file cũ + upload mới)...${NC}"
if git push -u origin _upload_tmp:main --force 2>&1; then
  echo -e "\n${BOLD}${GREEN}✔ Upload thành công! File cũ trên GitHub đã được thay thế.${NC}"
...
git branch -D _upload_tmp 2>/dev/null
