/**
 * includes/auto/tuongTac.js
 * Theo dõi tương tác người dùng theo ngày/tuần/tháng
 * Tự động gửi bảng xếp hạng top tương tác nhóm.
 *
 * Dữ liệu lưu tại: includes/data/tuongtac.json
 * Format:
 * {
 *   "groupId": {
 *     "userId": { "name": "...", "day": 0, "week": 0, "month": 0, "total": 0 }
 *   }
 * }
 */
...
function startTuongTac(api) {
  logInfo("[TuongTac] Đã khởi động theo dõi tương tác.");

  const startDailyTimer = () => {
    const msToMidnight = getNextMidnightMs();
    setTimeout(async () => {
    ...
    logInfo(`[TuongTac] Top ngày sẽ gửi sau ${Math.round(msToMidnight / 60000)} phút.`);
  };

  const startWeeklyTimer = () => {
    const msToMonday = getNextMondayMs();
    setTimeout(async () => {
    ...
    logInfo(`[TuongTac] Top tuần sẽ gửi sau ${Math.round(msToMonday / 3600000)} giờ.`);
  };

  const startMonthlyTimer = () => {
    const msToFirstOfMonth = getNextFirstOfMonthMs();
    setTimeout(async () => {
    ...
    logInfo(`[TuongTac] Top tháng sẽ gửi sau ${Math.round(msToFirstOfMonth / 3600000)} giờ.`);
  };
