...
const BACKUP_FILES = [
  { local: path.join(__dirname, "data", "credentials.json"), remote: "data/credentials.json" },
  { local: path.join(__dirname, "data", "keys.json"),        remote: "data/keys.json"        },
];
...
async function restoreFromGitHub(): Promise<void> {
...
async function runBackup(): Promise<{ success: boolean; ok: number; fail: number; skip: number; time: string }> {
  const now = new Date().toLocaleString("vi-VN", { timeZone: "Asia/Ho_Chi_Minh" });
  if (!GITHUB_TOKEN || !GITHUB_REPO) {
...
function scheduleBackup(intervalMs = 6 * 60 * 60 * 1000): void {
  if (!GITHUB_TOKEN || !GITHUB_REPO) {
