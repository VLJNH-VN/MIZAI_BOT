  ...
  run: async ({ api, event, args, send, threadID, prefix }) => {
    ...
    if (!sub) {
      return send(
        ...
        `${prefix}box rename <tên>   Đổi tên nhóm\n` +
        `${prefix}box owner @tag     Chuyển trưởng nhóm\n` +
        `${prefix}box avatar         Đổi ảnh nhóm (reply ảnh)\n` +
        `${prefix}box block @tag     Chặn thành viên\n` +
        `${prefix}box unblock @tag   Bỏ chặn thành viên\n` +
        `${prefix}box blocklist      Danh sách bị chặn\n` +
        `${prefix}box disperse       Giải tán nhóm ⚠️\n` +
        `${prefix}box upgrade        Nâng lên Cộng đồng`
      );
    }
